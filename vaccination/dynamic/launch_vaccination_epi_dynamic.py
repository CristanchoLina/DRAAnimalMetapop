...
import Epi_decision_functions_treatment as epidec
...
N0s = np.loadtxt('/home/maiage/lcristancho/SIMULATIONS/Centralised_Code/Cluster_25epidemic_treat50reps/fixed_parameters/N0s.txt')
L = len(N0s)
delta, nb_steps = np.loadtxt('/home/maiage/lcristancho/SIMULATIONS/Centralised_Code/Cluster_25epidemic_treat50reps/fixed_parameters/setting.txt')
nb_steps = int(nb_steps)
demo_params = np.loadtxt('/home/maiage/lcristancho/SIMULATIONS/Centralised_Code/Cluster_25epidemic_treat50reps/fixed_parameters/demo_params.txt')
theta_edges = np.loadtxt('/home/maiage/lcristancho/SIMULATIONS/Centralised_Code/Cluster_25epidemic_treat50reps/fixed_parameters/theta_edges.txt')



#Decision times(fct of nb of steps and duration decision)
farmers_duration = int(15/delta)
simul_list = np.array(range(0, nb_steps))
farmers_decision_times = simul_list[np.mod(simul_list, farmers_duration) == 0.0]
...
def model_full(variable_parameters_list, fixed_parameters_list = fixed_parameters_list):
    ...
    budget = 25 #int(L*0.10)

    # Epidemic evolution
    farmers_decision_times_i, social_decision_times_i, farmers_decisions_i, social_decisions_i,\
    ...
    #print(len(social_decision_times_i))     

    # Scores
    scores = scores_i

    # Output prop sum inf 
    I_intra = np.copy(I) #nb animaux infectes dans chaque troupeau à chaque instant
    Iintra = np.zeros((nb_steps, 1))
    for t in range(0,nb_steps):
        Iintra[t, 0] = np.sum(I_intra[t]) # nb danimaux infectés dans toute la metapop, a l'instant t
...
combinations_task_id = np.genfromtxt('/home/maiage/lcristancho/SIMULATIONS/Centralised_Code/Cluster_25epidemic_treat50reps/combinations/combinations_'+ task_id + '.txt', dtype='str') #task_id
...
nb_runs = 50
start_time = datetime.now()
combinations_task_id_shape = 1 #combinations_task_id.shape[0]
...
with open('scores_' + task_id + '.txt', 'w') as outfile:
...
# Write the array to disk
with open('decision_dynamic_' + task_id + '.txt', 'w') as outfile:
    # I'm writing a header here just for the sake of readability
    # Any line starting with "#" will be ignored by numpy.loadtxt
...
# Write the array to disk
with open('daily_prop_infected_farms_dynamic_' + task_id + '.txt', 'w') as outfile:
    # I'm writing a header here just for the sake of readability
    # Any line starting with "#" will be ignored by numpy.loadtxt
...
# Write the array to disk
with open('daily_infected_animals_dynamic_' + task_id + '.txt', 'w') as outfile:
    # I'm writing a header here just for the sake of readability
    # Any line starting with "#" will be ignored by numpy.loadtxt
...
np.savetxt('used_doses_I_number_original_shape_' + task_id + '.txt',
           used_doses_I_number_original_shape, fmt='%1.2f')

# Write the array to disk
with open('used_doses_I_number_' + task_id + '.txt', 'w') as outfile:
    # I'm writing a header here just for the sake of readability
    # Any line starting with "#" will be ignored by numpy.loadtxt
...
np.savetxt('used_doses_N_number_original_shape_' + task_id + '.txt',
           used_doses_N_number_original_shape, fmt='%1.2f')

# Write the array to disk
with open('used_doses_N_number_' + task_id + '.txt', 'w') as outfile:
    # I'm writing a header here just for the sake of readability
    # Any line starting with "#" will be ignored by numpy.loadtxt
