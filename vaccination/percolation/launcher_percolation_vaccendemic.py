...
combinations_task_id = np.genfromtxt('score_buget_list/combinations_'+ task_id + '.txt', dtype='str') #task_id
...
# In[19]:
np.savetxt('percolation_vacc_endemic_data/' + 'prop_infected_farms_end_' + task_id + '.txt', prop_infected_farms_end, fmt='%1.2f')
...
with open('percolation_vacc_endemic_data/' +'prop_vaccinated_' + task_id + '.txt', 'w') as outfile:
...
# Write the array to disk
with open('percolation_vacc_endemic_data/' +'wasted_doses_' + task_id + '.txt', 'w') as outfile:
    # I'm writing a header here just for the sake of readability
    # Any line starting with "#" will be ignored by numpy.loadtxt
