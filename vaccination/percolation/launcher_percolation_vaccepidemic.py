...
def model_full(variable_parameters_list, fixed_parameters_list = fixed_parameters_list):
    ...
    all_states_i, ventes_byfarm_i, achats_byfarm_i, scores_i = epidec.path(gamma, taus[0], initial_states,\
                                                                                    ...
                                                                                    parents_list,\
                                                                                    probs_exports,\
                                                                                    social_duration, budget, eff_reduceinfec, eff_protect, thetas, delta, nb_steps,\
...
# In[19]:
np.savetxt('percolation_vacc_epidemic_data/' + 'prop_infected_farms_end_' + task_id + '.txt', prop_infected_farms_end, fmt='%1.2f')
...
with open('percolation_vacc_epidemic_data/' +'prop_vaccinated_' + task_id + '.txt', 'w') as outfile:
...
# Write the array to disk
with open('percolation_vacc_epidemic_data/' +'wasted_doses_' + task_id + '.txt', 'w') as outfile:
    # I'm writing a header here just for the sake of readability
    # Any line starting with "#" will be ignored by numpy.loadtxt
