...
def model_full(variable_parameters_list, fixed_parameters_list = fixed_parameters_list):
    ...
    auc = np.trapz(prop_infected_farms_dynamic, dx=0.5)
    ...
    #Extinction time
    prop_infected_farms_dynamic = np.mean(I!=0, axis = 1)
    where_extinction = np.where(prop_infected_farms_dynamic == 0)[0]
    if not list(where_extinction) :
        extinction_time = np.NAN
    else:
        extinction_time = np.min(where_extinction)

    return prop_infected_farms_end, auc, used_doses_I_number, used_doses_N_number, extinction_time

# In[9]:


def eval_function(combinations):
    prop_infected_farms_end, auc, used_doses_I_number, used_doses_N_number , extinction_time  = model_full(combinations) 
    return prop_infected_farms_end, auc, used_doses_I_number, used_doses_N_number , extinction_time


combinations_task_id = np.genfromtxt('DRAAnimalMetapop/treatment/percolation/score_budget_list/combinations_'+ task_id + '.txt', dtype='str') #task_id
...
auc = np.zeros([nb_runs, combinations_task_id_shape])
...
extinction_time = np.zeros([nb_runs, combinations_task_id_shape])

# In[18]:

random.seed(1) # SEED
for i in range(0, nb_runs):
    prop_infected_farms_end[i], auc[i], used_doses_I_number[i], used_doses_N_number[i], extinction_time[i]  = eval_function(combinations_task_id)
    
    
# In[19]:
np.savetxt('DRAAnimalMetapop/treatment/percolation/percolation_treatment_endemic_data/'+'prop_infected_farms_end_' + task_id + '.txt', prop_infected_farms_end, fmt='%1.2f')

# In[19]:
np.savetxt('DRAAnimalMetapop/treatment/percolation/percolation_treatment_endemic_data/'+'auc_' + task_id + '.txt', auc, fmt='%1.2f')

# In[19]:
np.savetxt('DRAAnimalMetapop/treatment/percolation/percolation_treatment_endemic_data/'+'extinction_time_' + task_id + '.txt', extinction_time, fmt='%1.2f')
...
with open('DRAAnimalMetapop/treatment/percolation/percolation_treatment_endemic_data/'+'used_doses_I_number_' + task_id + '.txt', 'w') as outfile:
    # I'm writing a header here just for the sake of readability
    # Any line starting with "#" will be ignored by numpy.loadtxt
    outfile.write('DRAAnimalMetapop/treatment/percolation/# Array shape: {0}\n'.format(used_doses_I_number_original_shape))
    ...
    for data_slice in used_doses_I_number:
        ...
        outfile.write('DRAAnimalMetapop/treatment/percolation/# New step \n')
...
# Write the array to disk
with open('DRAAnimalMetapop/treatment/percolation/percolation_treatment_endemic_data/'+'used_doses_N_number_' + task_id + '.txt', 'w') as outfile:
    # I'm writing a header here just for the sake of readability
    # Any line starting with "#" will be ignored by numpy.loadtxt
